...
let process_preamble ((ds, p):prog) =
  ...
  let (x, t, es)  = p in
  let tree        = Rtree.create_rtree s g env t !match_count in
  (s, g, env, x, t, es, tree)

let print_graph (p:prog) : prog =
  let (_, _, _, _, _, _, tree) = process_preamble p in
  Printf.printf "%s\nsize = %d\n" (Rtree.pp_rtree tree) (Rtree.rtree_size tree);
  p

let enumerate_progs (p:prog) : prog =
  let (s, _, env, x, t, _, tree) = process_preamble p in
...
(*let collect_data (p:prog) : prog =
  let (time, (x, vs, e)) = Util.time_action ~f:(fun _ ->
    let (s, _, env, x, _, _, tree) = process_preamble p in
...
let synthesize_prog (p:prog) : prog =
  let (s, _, env, x, t, _, tree) = process_preamble p in
  begin match Synth.synthesize s env tree (fun _ -> 0.) with
  | Some e ->
