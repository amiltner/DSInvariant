...
Module Type SetType .
 ...
 Parameter insert: key -> table -> table.
 Parameter delete: key -> table -> table.
 Parameter lookup: key -> table -> bool.
 Axiom gempty: forall k,   (* get-empty *)
       lookup k empty = false.
 Axiom gss: forall k t,      (* get-set-same *)
      lookup k (insert k t) = true.
 Axiom gso: forall k t,    (* get-set-other *)
      lookup k (delete k t) = true.
End SetType.
...
Inductive SearchList' : nat -> list nat -> Prop :=
| SL_E : forall v, SearchList' v nil 
| SL_C : forall h t v, SearchList' h t ->
                    v > h ->
                    SearchList' v (cons h t).

Inductive SearchList : list nat -> Prop :=
| SL_intro : forall h t, SearchList' h t -> SearchList (cons h t)
| SL_empty : SearchList nil.

Require Import Nat.

Fixpoint insert (t : list nat) (x : nat) : list nat :=
  match t with
  | nil => cons x nil
  | cons x' t' =>
    if ltb x x' then
      cons x' (insert t' x)
    else if ltb x x' then
           cons x t
         else
           t
  end.
...
Theorem empty_searchlist: SearchList nil.
Proof. intros. constructor. Qed.

Theorem insert_searchlist: forall x t, SearchList t -> SearchList (insert t x).
Proof. intros. destruct t. simpl. constructor. constructor. simpl. Qed.

Module MapsTable <: SetType.
 Definition table := {x | SearchList x}.
 Definition empty : table :=
 exist SearchList nil empty_searchlist.
