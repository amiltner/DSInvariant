...
let rec rtree_size (t:rtree) : int =
  let rnode_size n =
    ...
    | SRcd ts -> List.fold_left ~f:(fun acc (l,t) -> rtree_size t + acc) ~init:0 ts
    | SUnit -> 0
  in
...
and build_lines_rnode (k:int) (tt:typ) (n:rnode) : pretty_line list =
  ...
  | SRcd ts ->
      ...
      (k, s) :: List.concat_map ~f:(fun (l,t) -> build_lines_rtree (k+1) t) ts
  | SUnit ->
...
and create_match (s:Sigma.t) (g:Gamma.t) (env:env) (t:typ)
                     (ws:world list) (matches:int) ((e, evs):exp * (eval_val list))
                     : rmatch option =
    ...
    let trees = List.map ~f:(fun (c, p, g, ws) ->
...
and create_non_matches (s:Sigma.t) (g:Gamma.t) (env:env) (t:typ) (vs:world list)
  ...
  | TRcd ts ->
      ...
      let labels_match = List.for_all values ~f:(fun v ->
        begin match v with
          | VRcd vs' -> Util.fst_assoc vs' = tlabels
          | _ -> false
        end) in
